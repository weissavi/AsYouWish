...
def validate_mood(mood):
    if mood not in valid_moods:
        print(f"[⚠️] Unknown mood: '{mood}'. Defaulting to 'neutral'.")
        return "neutral"
    return mood
...
combined_prompt = persona.get("free_will_prompt", "").format(mood=mood)

persona_prompt = {"role": "system", "content": combined_prompt}
        ...
        while True:
            ...
            try:
                ...
                if 'last_user_input_time' in globals():
                    delta = datetime.datetime.now() - last_user_input_time
...
def main():
    ...
    while True:
        ...
        last_user_input_time = datetime.datetime.now()  
...
if __name__ == "__main__":
    if persona.get("has_free_will", False):
        run_free_will()
    main()
